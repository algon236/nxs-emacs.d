;;; emacs-nxs-rate.el --- Cryptocurrency and fiat exchange rate viewer  -*- lexical-binding: t; -*-
;;
...
(use-package emacs-nxs-rate
  :ensure nil
  :no-require t
  :defer t
  :init
  (setq emacs-nxs-rate-crypto "BTC")
  (setq emacs-nxs-rate-fiat "USD")

  (defun emacs-nxs/rate-buffer (&optional which)
...
WHICH may be:
  ...
  nil   → fetch both"
  (interactive)
  (let* ((crypto (shell-quote-argument emacs-nxs-rate-crypto))
         (fiat   (shell-quote-argument emacs-nxs-rate-fiat))
    ...
    (pcase which
      ('url1
       (emacs-nxs--fetch-rate url1 buffer))
      ('url2
       (emacs-nxs--fetch-rate url2 buffer t))
      (_
       (emacs-nxs--fetch-rate url1 buffer)
       (emacs-nxs--fetch-rate url2 buffer t)))))

  (defun emacs-nxs--fetch-rate (cmd buffer &optional second)
...
(provide 'emacs-nxs-rate)
;;; emacs-nxs-rate.el ends here
