;;; emacs-nxs-erc-image.el --- Inline images in ERC chat buffers  -*- lexical-binding: t; -*-
;;
...
(use-package emacs-nxs-erc-image
  :if emacs-nxs-enable-erc-image
  :ensure nil
  :no-require t
  :after erc
  :config
  (defvar erc-image-cache-directory
    ;; Prefer Emacs NXS's central cache-path registry when available,
    ...
    ;; helper.
    (if (fboundp 'emacs-nxs--cache-path)
        (emacs-nxs--cache-path 'erc-image-cache-directory)
...
(provide 'emacs-nxs-erc-image)
;;; emacs-nxs-erc-image.el ends here
