;;; calendar-danish.el -*- lexical-binding: t; mode: emacs-lisp; coding: utf-8 -*-
...
;;; Code:
;; maccalfw: A calfw interface for macOS Calendar.
;; Pakken ligger på GitHub og installeres eksplicit med package-vc.
;; Vi bruger bevidst hverken use-package-nøgleordet :straight eller :vc,
;; så konfigurationen virker med den use-package-version, Emacs indlæser.
(defconst emacs-nxs-maccalfw-url
  "https://github.com/haji-ali/maccalfw.git"
  "Git-adresse til pakken maccalfw.")

(defun emacs-nxs-install-maccalfw ()
  "Installér eller opdatér maccalfw med Emacs' indbyggede package-vc."
  (interactive)
  (unless (fboundp 'package-vc-install)
    (user-error "Denne Emacs-version understøtter ikke package-vc-install"))
  (package-vc-install emacs-nxs-maccalfw-url)
  (message "maccalfw er installeret eller opdateret"))

(defun emacs-nxs-maccalfw-open ()
  "Åbn maccalfw; tilbyd installation, hvis pakken endnu ikke findes."
  (interactive)
  (unless (require 'maccalfw nil t)
    (if (yes-or-no-p "maccalfw er ikke installeret. Installér pakken nu? ")
        (progn
          (emacs-nxs-install-maccalfw)
          (require 'maccalfw))
      (user-error "maccalfw er ikke installeret")))
  (call-interactively #'maccalfw-open))

;; latitude and longgitude is defined i ~/etc/private.el
;; (defvar calendar-latitude 55.93843)
;; (defvar calendar-longitude 12.48938)
...
(defun cal/day-of-week (year month day)
  “Returnerer ugedag som tal 0..6, hvor mandag=0.”
   (let* ((Y (if (< month 3) (1- year) year))
          (m (1+ (mod (+ month 9) 12)))
          (y (mod Y 100))
          (c (/ Y 100)))
     (mod (+ day(floor (- (* 26 m) 2) 10)y(/ y 4)(/ c 4)(* -2 c)6))))
