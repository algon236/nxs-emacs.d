;;; emacs-nxs-replace-as-diff.el --- Multi-file regexp replace with diff preview  -*- lexical-binding: t; -*-
;;
...
(use-package emacs-nxs/replace-regexp-as-diff
  :ensure nil
  :no-require t
  :defer t
  :init
  ...
  ;;           subfolders (`emacs-nxs/multi-file-replace-regexp-as-diff-with-glob')
  ;;       2.) Use it with marked files and or directories in dired
  ;;           (`emacs-nxs/dired-do-replace-regexp-as-diff')


  ;; `M-x emacs-nxs/multi-file-replace-regexp-as-diff-with-glob RET'
  ;;
  ...
  (defun emacs-nxs/multi-file-replace-regexp-as-diff-with-glob (dir regexp to-string &optional delimited glob-pattern)
...
GLOB-PATTERN is the glob pattern to match files (e.g., \"*.el\")."
  ...
  (defun emacs-nxs/expand-directories (items)
...
Directories themselves are excluded from the final list."
  ...
  (defun emacs-nxs/dired-do-replace-regexp-as-diff (from to &optional delimited)
...
you can later apply as a patch after reviewing the changes."
    ...
    (let* ((marked-items (dired-get-marked-files)) ; Include directories in the list
           (files (emacs-nxs/expand-directories marked-items)))
      (if files
          (progn
            (multi-file-replace-regexp-as-diff files from to delimited))
        (message "No files found in marked items.")))))

(provide 'emacs-nxs-replace-as-diff)
;;; emacs-nxs-replace-as-diff.el ends here
